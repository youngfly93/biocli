  ...
  it('serializes manifest-facing command metadata needed by discovery', async () => {
    ...
    const entries = await loadTsManifestEntries(filePath, 'manifest-test', async () => {
      registerCommand({
        ...
        database: 'aggregate',
        strategy: Strategy.PUBLIC,
        args: [{ name: 'study', help: 'Study ID', producedBy: ['manifest-test/studies'] }],
        columns: ['step', 'status'],
        timeoutSeconds: 300,
        ...
        whenToUse: 'Use when preparing a workflow workspace from a chosen dataset.',
        readOnly: false,
    ...
    expect(entries).toEqual([
      {
        ...
        database: 'aggregate',
        strategy: 'public',
        args: [{ name: 'study', type: 'str', required: false, help: 'Study ID', producedBy: ['manifest-test/studies'] }],
        columns: ['step', 'status'],
        defaultFormat: 'json',
        timeout: 300,
        requiredEnv: [{ name: 'BIOCLI_API_TOKEN', help: 'Set the token first.' }],
